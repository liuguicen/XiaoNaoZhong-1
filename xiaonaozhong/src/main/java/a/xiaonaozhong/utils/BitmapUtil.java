...
public class BitmapUtil {
	...
	public Bitmap getIconInApp(Bitmap bitmap, String title,Boolean open) {
		...
		//获取已有的Drawable图片
		Resources res = context.getResources();
		int bitmapCanvasSize = (int)(((int) context.getResources().getDimension(
				android.R.dimen.app_icon_size))*1.8);
		Bitmap bitmapCanvas = Bitmap.createBitmap(bitmapCanvasSize, bitmapCanvasSize,
				Config.ARGB_8888);//位图画板// 初始化画宽度
		Canvas canvas = new Canvas(bitmapCanvas);
		if (bitmap == null)
			bitmap = BitmapFactory.decodeResource(res, R.mipmap.naozhong_icon);
		bitmap=createFramedPhoto(bitmapCanvasSize, bitmapCanvasSize, bitmap, 25);
		//设置用于缩放
		Rect srcRect = new Rect(0, 0, bitmap.getWidth(), bitmap.getHeight());//原图片的大小
		Rect objRect = new Rect(0, 0, bitmapCanvasSize, bitmapCanvasSize);//底板的大??
		...
		countPaint.setTypeface(Typeface.SANS_SERIF);
		canvas.drawText(title, 2, 11 * bitmapCanvasSize / 16,
				countPaint);
		return bitmapCanvas;
	}
